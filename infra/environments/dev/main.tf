...
resource "azurerm_resource_group" "platform" {
  ...
  lifecycle {
    precondition {
      condition     = var.environment == "dev" && !can(regex("(?i)prod", var.environment))
      error_message = "Only the development environment is permitted by this composition."
    }
    precondition {
      condition     = data.azurerm_client_config.current.subscription_id == var.subscription_id
      error_message = "Authenticated Azure subscription does not match the explicitly approved non-production subscription_id."
    }
    precondition {
      condition     = contains(var.approved_nonprod_subscription_ids, var.subscription_id)
      error_message = "subscription_id is not present in the independently supplied non-production subscription allow-list."
    }
    precondition {
